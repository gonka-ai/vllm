...
class TestChatPriorityGating:
    ...
    def test_05_chat_completions_work_after_poc(self):
        ...
        def assert_not_degenerate(text: str):
            compact = text.strip()
            assert compact, "Empty response after PoC"
            if len(compact) < 24:
                return
            chunks = [compact[i:i + 4] for i in range(0, len(compact) - 3, 4)]
            max_repeats = max(chunks.count(chunk) for chunk in set(chunks))
            assert max_repeats <= max(6, int(len(chunks) * 0.6)), (
                f"Degenerate repeated output after PoC: {text!r}"
            )
