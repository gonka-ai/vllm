...
docker run --runtime nvidia --gpus all \
    ...
    -p 8005:8005 \
    --ipc=host \
    gcr.io/decentralized-ai/vllm:0.7.3.post1 \
    --model unsloth/llama-3-8b-Instruct \
    --port 8005



curl -X POST "0.0.0.0:8005/v1/chat/completions" -H 'accept: application/json' -H 'Content-Type: application/json' -d '{
    "model": "unsloth/llama-3-8b-Instruct",
    "messages": [
...
curl -X POST "0.0.0.0:8005/v1/chat/completions" -H 'accept: application/json' -H 'Content-Type: application/json' -d '{
    ...
    "logprobs": 1,
    "top_logprobs": 3,
    "enforced_str": "one one one one one one one one"
  }' | jq

curl -X POST "0.0.0.0:8080/api/v1/inference/up" \
...
uvicorn blockchain_api:app --host 127.0.0.1 --port 8000


docker run -it -v /mnt/ramdisk/tamaz/mlnode/packages/train/tests/unit:/app/packages/train/tests/unit train-integration-tests \
bash -c "cd /app/packages/train/tests/unit/storage && uvicorn blockchain_api:app --host 127.0.0.1 --port 8000"