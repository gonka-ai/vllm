    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            for seq_group in seq_groups:
                ...
                if len(output_token_ids) < len(sampling_params.enforce_token_ids):
                    enforced_token_ids.append(
                        sampling_params.enforce_token_ids[len(output_token_ids)]
                    )
                else:
                    enforced_token_ids.append(sampling_params.stop_token_ids[0])
                

            if sampled_token_ids_tensor is not None and enforced_token_ids:
                sampled_token_ids_tensor[
                    long_sample_indices] = torch.tensor(enforced_token_ids, device=logprobs.device)
            ...
            for seq_group in seq_groups:
                if seq_group.is_prompt:
                    ...
                    num_samples = max(
                        num_samples,
                        sampling_params.best_of if sampling_params.best_of else -1
                    )
        else:
