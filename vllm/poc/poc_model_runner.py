...
def _detach_layer_hooks(worker):
    existing_hook = getattr(worker, "_poc_layer_hooks", None)
    if existing_hook is not None:
        existing_hook.detach()
...
def _select_poc_kv_scratch(
    ...
    allocate a copy.
    """
    # v0.23.0's V2 GPU runner keeps more persistent state around KV cache
    # tensors. Reusing those tensors as PoC input embeddings can leave later
    # inference in a degenerate state after repeated PoC rounds, so keep PoC
    # scratch separate for correctness.
    return None
