...
async def poc_request(
    self, action: str, payload: dict, timeout_ms: int = 60000
) -> dict:
    ...
    if output_processor is not None and output_processor.has_unfinished_requests():
        ...
        if request_ids:
            logger.info(
                "PoC aborting %d in-flight inference request(s)", len(request_ids)
            )
    ...
    try:
        ...
        try:
            ...
            hidden_size = 8192
            logger.warning(
                f"Could not get hidden_size from config, using default: {hidden_size}"
            )
