...
POC_CALLBACK_MAX_ARTIFACTS = int(
    os.environ.get("POC_CALLBACK_MAX_ARTIFACTS", "1000000")
)
            ...
            while not self.stop_event.is_set():
                ...
                if self._pending_payload:
                    ...
                    if success:
                        if retry_attempt > 1:
                            logger.info(
                                f"Callback to {self.callback_url} succeeded after {retry_attempt} attempts"
                            )
                        self._pending_payload = None
                        backoff = POC_CALLBACK_RETRY_BACKOFF_SEC
                        retry_attempt = 0
                        ...
                        if retry_attempt == 1 or retry_attempt % 10 == 0:
                            logger.warning(
                                f"Callback to {self.callback_url} failed (attempt {retry_attempt}/{POC_CALLBACK_MAX_RETRIES}, next backoff {backoff:.1f}s)"
                            )
...
class CallbackQueue:
    ...
    async def _send_with_retry(self, url: str, path: str, payload: dict) -> bool:
        ...
        async with self._semaphore:
                ...
                try:
                    async with self._session.post(
                        ...
                        if resp.status < 400:
                            ...
                            return True
                        logger.warning(
                            f"Callback to {url_path} HTTP {resp.status} (attempt {attempt}/{POC_CALLBACK_MAX_RETRIES})"
                        )
                except Exception as e:
                    logger.warning(
                        f"Callback to {url_path} failed: {e} (attempt {attempt}/{POC_CALLBACK_MAX_RETRIES})"
                    )
            ...
            logger.error(
                f"Callback to {url_path} failed after {POC_CALLBACK_MAX_RETRIES} attempts, giving up"
            )
            return False
